...
import {Button, Navbar, Heading, Section, Box, Level, Form, Menu, Columns} from 'react-bulma-components';
        ...
        <Columns.Column size={"one-quarter"}>
          ...
          <Menu>
            <Menu.List title="Shortcuts">
              ...
              <Link to={"/"}><Menu.List.Item active>Manage Shortcuts</Menu.List.Item></Link>
              <Menu.List.Item>View Visits</Menu.List.Item>
            </Menu.List>

            <Menu.List title="Administration">
              <Link to={"/users"}><Menu.List.Item>Manage Users</Menu.List.Item></Link>
            </Menu.List>
          </Menu>
          </Box>
  ...
  shortcuts && shortcuts.map((s: any) => (
    <Box style={{ marginBottom: 10, border: 3, borderSpacing: "solid" }}>
      <Columns style={{flexDirection: "row-reverse"}}>
        <Columns.Column>
          <Heading size={4}><span style={{color: "grey"}}>dxe.io/</span>{s.Code}</Heading>
          <Heading subtitle size={6}><a href={s.URL} target={"_blank"}>{s.URL}</a></Heading>
        </Columns.Column>
